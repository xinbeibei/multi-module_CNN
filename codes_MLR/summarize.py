...
if len(sys.argv) != 6:
    ...
    print 'Usage:', sys.argv[0], '<path_to_tf_list_file> <input_path> <path_to_r_script> <output_path> <feature_list_location>'
    sys.exit(1)
...
# ++
f = open(output_path+'summary_r2.txt', 'w') # fixed filename "summary.txt"
f.write(' '.join(header))
f.write('\n')
f.close()

cmd = 'R --no-restore --no-save --args ' + input_path + tf + ' ' + output_path + ' ' + feature_list_location + ' < ' + r_script + ' 2>&1 >/dev/null'
os.system(cmd)
