...
def seq_and_HM_CNN(lr1, lr2, input_len_seq,input_channel_seq, input_len_hm, input_channel_hm, nb_filter_seq=[20], \
    ...
    if add_seq:
        ...
        w_reg_seq = kr.L1L2(l1=lr1, l2=lr2)
        seq_model.add(keras.layers.Conv1D(input_shape=(input_len_seq,input_channel_seq),
                                                       filters=nb_filter_seq[0],
                                                       activation='relu',
                                                       padding='same',
                                                       kernel_initializer='glorot_uniform',
                                                       kernel_regularizer=w_reg_seq,
                                                       kernel_size=filter_len_seq))
        seq_model.add(keras.layers.Activation("relu"))                                                   
        seq_model.add(keras.layers.MaxPooling1D(pool_size=pool_len))
        if len(nb_filter_seq) > 1:
            for nb_filter in nb_filter_seq[1:]:
                seq_model.add(keras.layers.Conv1D(filters=nb_filter,
                                                       padding='same',
                                                       kernel_size=filter_len_seq))
                seq_model.add(keras.layers.MaxPooling1D(pool_size=pool_len))  
    ...
    if add_hm:       
        ...
        w_reg_hm = kr.L1L2(l1=lr1, l2=lr2)
        hm_model.add(keras.layers.Conv1D(input_shape=(input_len_hm,input_channel_hm),
                                                       filters=nb_filter_hm[0],
                                                       activation='relu',
                                                       padding='same',
                                                       kernel_initializer='glorot_uniform',
                                                       kernel_regularizer=w_reg_hm,
                                                       kernel_size=filter_len_hm))
        hm_model.add(keras.layers.Activation("relu")) 
        hm_model.add(keras.layers.MaxPooling1D(pool_size=pool_len))
        if len(nb_filter_hm) > 1:
            for nb_filter in nb_filter_hm[1:]:
                hm_model.add(keras.layers.Conv1D(filters=nb_filter,
                                                       padding='same',
                                                       kernel_size=filter_len_hm))
                hm_model.add(keras.layers.MaxPooling1D(pool_size=pool_len))   
        hm_model.add(keras.layers.Flatten())
    
    if add_seq and add_hm:
        merged = keras.layers.concatenate([seq_model.output, hm_model.output], axis=-1)        
        # merged = keras.models.Merge([seq_model, hm_model], mode= 'concat')
        for dd in dense_dim:    
            dense_layers = keras.layers.Dense(dd, activation='linear',
                                        kernel_initializer='glorot_uniform', use_bias=True)(merged)
            dense_layers = keras.layers.Activation("relu")(dense_layers)
            dense_layers = keras.layers.Dropout(dropout)(dense_layers)
        out = keras.layers.Dense(2, activation='softmax',
                                kernel_initializer='glorot_uniform', use_bias=True)(dense_layers)
        model = keras.models.Model([seq_model.input, hm_model.input], [out])
    elif add_seq:
        model = seq_model
        for dd in dense_dim:    
            model.add(keras.layers.Dense(dd, activation='linear',
...
def train_encode(outdir, onehot, seqs, hm, y, lr_file, tf_len, nfold=3, batch_size=128, \
    ...
    for i, param in enumerate(params):
        ...
        opt = Adam(lr=lr_backprop)
        model.compile(optimizer=opt, loss='binary_crossentropy', metrics=['accuracy', precision, recall])   #for regression models
        ...
        for j in np.arange(nfold):
            ...
            if add_hm and add_seq:
                model.fit([onehot_tr, hm_tr], labels, validation_data =([onehot_val, hm_val], val_labels), callbacks=[earlystop], batch_size = batch_size, epochs = nb_epoch, class_weight=d_class_weight, verbose = 2)
            elif add_seq:
                model.fit(onehot_tr, labels, validation_data =(onehot_val, val_labels), callbacks=[earlystop], batch_size = batch_size, epochs = nb_epoch, class_weight=d_class_weight, verbose = 2)

            if add_hm and add_seq: 
               score = model.evaluate([onehot_val, hm_val], val_labels, batch_size)[1]
            elif add_seq:
               score = model.evaluate(onehot_val, val_labels, batch_size)[1]  
            perfs[i,j] = score
    ...
    outfile = open(outdir + "/train_perf" + suffix + ".txt", 'w')
    for k in np.arange(num_to_save):
        lr_conv1, lr_conv2, lr_backprop = np.float(best_params[k][0]), np.float(best_params[k][1]), np.float(best_params[k][2])
        model = seq_and_HM_CNN(lr_conv1, lr_conv2, input_len_seq, input_channel_seq, input_len_hm, input_channel_hm,\
        filter_len_seq = int(tf_len) +2, filter_len_hm = int(tf_len)+2, add_seq = add_seq, add_hm = add_hm)
        ...
        for z in np.arange(ntrial):
            ...
            if add_hm and add_seq:
                model.fit([onehot, hm], labels, validation_split=0.2, callbacks=[earlystop], batch_size = batch_size, epochs = nb_epoch, class_weight=d_class_weight, verbose = 2)
            elif add_seq:
                model.fit(onehot, labels, validation_split=0.2, callbacks=[earlystop], batch_size = batch_size, epochs = nb_epoch, class_weight=d_class_weight, verbose = 2)
            
            if add_hm and add_seq: 
                score = model.evaluate([onehot, hm], labels, batch_size)[1]
            elif add_seq:
                score = model.evaluate(onehot, labels, batch_size)[1]
                
            if score > train_perfs:
                train_perfs = score
                outmodel = open(outdir + "model_"+ ",".join(best_params[k]) + suffix + ".json", 'w')
                outmodel.write(model.to_json())
                outmodel.close()
                model.save_weights(outdir + "model_" + ",".join(best_params[k]) + suffix + ".h5")        
        outfile.write(",".join(best_params[k]) + "\t" + str(train_perfs) + "\n")
        print (",".join(best_params[k]) + "\t" + str(train_perfs) + "\n")
    outfile.close()
...
def predict_encode(outdir, param_list, onehot_test, seqs_test, hm_test, y_test, batch_size = 128, add_seq=True, add_hm=True):
    ...
    if add_hm and add_seq:
        ...
        suffix = "_seq"
    outfile = open(outdir + "/test_perf" + suffix + ".txt", 'w')

    for i, param in enumerate(params):
        lr_backprop = np.float(param[2])
        print ("Start loading model..." + "for parameter " + str(i) + ", "+ ",".join(param) + suffix + "\n")
        model = load_model(outdir + "/model_"+ ",".join(param) + suffix + ".json", outdir + "/model_" + ",".join(param) + suffix + ".h5")
        ...
        if add_hm and add_seq: 
            score = model.evaluate([onehot_test, hm_test], test_labels, batch_size)[1]
        elif add_seq:
            score = model.evaluate(onehot_test, test_labels, batch_size)[1]
...
def visualize(filename_prefix, matrix, width, title=''):
    ...
    + ' -p ' + str(width) + 'x350 -u \'' + title + '\' -H \'xaxis,fineprint,ends\''
    os.system(cmd)
